const exploreLayout = (() => {
    ...
    const showNewsList = (newsList) => {
        ...
        newsList.forEach(news => {
            const isLiked = news.liked ?? false;
            const isBookmarked = news.bookmarked ?? false;
            const likeCount = news.likeCount ?? 0;
                ...
                <div class="trend" role="link" tabindex="0" data-news-id="${news.id}">
                    <div class="trend-inner">
                        ...
                        <div class="trend-actions">
                            <button type="button"
                                class="tweet-action-btn tweet-action-btn--like trend-like${isLiked ? ' active' : ''}"
                                data-news-id="${news.id}"
                                data-action="news-like"
                                aria-label="좋아요 ${likeCount}">
                                <svg viewBox="0 0 24 24" aria-hidden="true" class="tweet-action-icon"><g><path d="${isLiked
                                    ...
                                    data-path-active="M20.884 13.19c-1.351 2.48-4.001 5.12-8.379 7.67l-.503.3-.504-.3c-4.379-2.55-7.029-5.19-8.382-7.67-1.36-2.5-1.41-4.86-.514-6.67.887-1.79 2.647-2.91 4.601-3.01 1.651-.09 3.368.56 4.798 2.01 1.429-1.45 3.146-2.1 4.796-2.01 1.954.1 3.714 1.22 4.601 3.01.896 1.81.846 4.17-.514 6.67z"></path></g></svg>
                                <span class="tweet-action-count">${likeCount}</span>
                            </button>
                            <button type="button"
                                class="tweet-action-btn tweet-action-btn--bookmark trend-bookmark${isBookmarked ? ' active' : ''}"
                                data-news-id="${news.id}"
                                data-action="news-bookmark"
                                aria-label="북마크">
                                <svg viewBox="0 0 24 24" aria-hidden="true" class="tweet-action-icon"><g><path d="${isBookmarked
                                    ? 'M4 4.5C4 3.12 5.119 2 6.5 2h11C18.881 2 20 3.12 20 4.5v18.44l-8-5.71-8 5.71V4.5z'
                                    : 'M4 4.5C4 3.12 5.119 2 6.5 2h11C18.881 2 20 3.12 20 4.5v18.44l-8-5.71-8 5.71V4.5zM6.5 4c-.276 0-.5.22-.5.5v14.56l6-4.29 6 4.29V4.5c0-.28-.224-.5-.5-.5h-11z'}"
                                    data-path-inactive="M4 4.5C4 3.12 5.119 2 6.5 2h11C18.881 2 20 3.12 20 4.5v18.44l-8-5.71-8 5.71V4.5zM6.5 4c-.276 0-.5.22-.5.5v14.56l6-4.29 6 4.29V4.5c0-.28-.224-.5-.5-.5h-11z"
                                    data-path-active="M4 4.5C4 3.12 5.119 2 6.5 2h11C18.881 2 20 3.12 20 4.5v18.44l-8-5.71-8 5.71V4.5z"></path></g></svg>
                            </button>
                        </div>
                    </div>
                </div>
