...
window.onload = () => {
  ...
  function getRecentEmojis() { try { const p = JSON.parse(window.localStorage.getItem(emojiRecentsKey) ?? '[]'); return Array.isArray(p) ? p : []; } catch { return []; } }
  function saveRecentEmoji(emoji) { const recent = getRecentEmojis().filter(i => i !== emoji); recent.unshift(emoji); try { window.localStorage.setItem(emojiRecentsKey, JSON.stringify(recent.slice(0, maxRecentEmojis))); } catch { return; } }
  ...
  function renderTagResults(users) {
    ...
    if (!hasQuery) { replyTagSearchInput.setAttribute('aria-expanded','false'); replyTagSearchInput.removeAttribute('aria-controls'); replyTagResults.removeAttribute('role'); replyTagResults.removeAttribute('id'); replyTagResults.innerHTML = ''; return; }
    replyTagSearchInput.setAttribute('aria-expanded','true'); replyTagSearchInput.setAttribute('aria-controls','community-tag-results');
    replyTagResults.setAttribute('role','listbox'); replyTagResults.id = 'community-tag-results';
  ...
  function openTagPanel() { if (!composeView || !replyTagView || !isReplyImageSet()) return; closeEmojiPicker(); pendingTaggedUsers = cloneTaggedUsers(selectedTaggedUsers); composeView.hidden = true; replyTagView.hidden = false; if (replyTagSearchInput) replyTagSearchInput.value = ''; renderTagChipList(); renderTagResults([]); window.requestAnimationFrame(() => replyTagSearchInput?.focus()); }
  function closeTagPanel({ restoreFocus = true } = {}) { if (!composeView || !replyTagView || replyTagView.hidden) return; replyTagView.hidden = true; composeView.hidden = false; pendingTaggedUsers = cloneTaggedUsers(selectedTaggedUsers); if (replyTagSearchInput) replyTagSearchInput.value = ''; renderTagChipList(); renderTagResults([]); if (restoreFocus) window.requestAnimationFrame(() => { replyUserTagTrigger && !replyUserTagTrigger.hidden ? replyUserTagTrigger.focus() : replyEditor?.focus(); }); }
  ...
  function openMediaEditor() { if (!composeView || !replyMediaView || !isReplyImageSet()) return; closeEmojiPicker(); pendingReplyMediaEdits = cloneReplyMediaEdits(replyMediaEdits); activeReplyMediaIndex = 0; composeView.hidden = true; replyMediaView.hidden = false; renderMediaEditor(); window.requestAnimationFrame(() => replyMediaAltInput?.focus()); }
  function closeMediaEditor({ restoreFocus = true, discardChanges = true } = {}) { if (!composeView || !replyMediaView || replyMediaView.hidden) return; if (discardChanges) pendingReplyMediaEdits = cloneReplyMediaEdits(replyMediaEdits); replyMediaView.hidden = true; composeView.hidden = false; if (restoreFocus) window.requestAnimationFrame(() => { replyMediaAltTrigger && !replyMediaAltTrigger.hidden ? replyMediaAltTrigger.focus() : replyEditor?.focus(); }); }
  ...
  function getFilteredLocations() {
    ...
    if (cachedLocationNames.length === 0 && replyLocationList) cachedLocationNames = Array.from(replyLocationList.querySelectorAll('.tweet-modal__location-item-label')).map(i => getTextContent(i)).filter(Boolean);
  ...
  function renderLocationList() {
    ...
    replyLocationList.innerHTML = locs.map(loc => { const sel = pendingLocation === loc; return `<button type="button" class="tweet-modal__location-item" role="menuitem"><span class="tweet-modal__location-item-label">${loc}</span><span class="tweet-modal__location-item-check">${sel ? '<svg viewBox="0 0 24 24" aria-hidden="true"><g><path d="M9.64 18.952l-5.55-4.861 1.317-1.504 3.951 3.459 8.459-10.948L19.4 6.32 9.64 18.952z"></path></g></svg>' : ''}</span></button>`; }).join('');
  }
  function openLocationPanel() { if (!composeView || !replyLocationView) return; closeEmojiPicker(); pendingLocation = selectedLocation; composeView.hidden = true; replyLocationView.hidden = false; if (replyLocationSearchInput) replyLocationSearchInput.value = ''; renderLocationList(); syncLocationUI(); window.requestAnimationFrame(() => replyLocationSearchInput?.focus()); }
  function closeLocationPanel({ restoreFocus = true } = {}) { if (!composeView || !replyLocationView || replyLocationView.hidden) return; replyLocationView.hidden = true; composeView.hidden = false; if (replyLocationSearchInput) replyLocationSearchInput.value = ''; pendingLocation = selectedLocation; renderLocationList(); syncLocationUI(); if (restoreFocus) window.requestAnimationFrame(() => replyEditor?.focus()); }
  ...
  function resetReplyAttachment() { clearAttachedReplyFileUrls(); attachedReplyFiles = []; pendingAttachmentEditIndex = null; resetTaggedUsers(); syncReplyMediaEditsToAttachments(); if (replyFileInput) replyFileInput.value = ''; if (replyAttachmentMedia) replyAttachmentMedia.innerHTML = ''; if (replyAttachmentPreview) replyAttachmentPreview.hidden = true; }
  ...
  function renderReplyAttachment() {
    ...
    replyAttachmentPreview.hidden = false; createReplyAttachmentUrls();
  ...
  function handleReplyFileChange(e) {
    ...
    if (pendingAttachmentEditIndex !== null) { if (!rep) { pendingAttachmentEditIndex = null; return; } if (rep.type.startsWith('video/')) { attachedReplyFiles = [rep]; } else { const ed = isReplyVideoSet() ? [] : [...attachedReplyFiles]; attachedReplyFiles = ed.length === 0 ? [rep] : ((ed[pendingAttachmentEditIndex] = rep), ed.slice(0, maxReplyImages)); } pendingAttachmentEditIndex = null; renderReplyAttachment(); syncReplySubmitState(); return; }
  ...
  function saveReplySelection() {
    ...
    const range = sel.getRangeAt(0); if (!replyEditor.contains(range.commonAncestorContainer)) return;
    savedReplySelection = range.cloneRange(); savedReplySelectionOffsets = getReplySelectionOffsets(range);
  }
  function getReplySelectionOffsets(range) {
    ...
    const start = pre.toString().length; return { start, end: start + range.toString().length };
  }
  function resolveReplySelectionPosition(targetOffset) {
    ...
    while (node) { lastTextNode = node; const length = node.textContent?.length ?? 0; if (remaining <= length) return { node, offset: remaining }; remaining -= length; node = walker.nextNode(); }
  ...
  function buildReplySelectionRangeFromOffsets(offsets) {
    ...
    const range = document.createRange(); range.setStart(startPos.node, startPos.offset); range.setEnd(endPos.node, endPos.offset); return range;
  }
  function restoreReplySelection() {
    ...
    if (!range) return false; sel.removeAllRanges(); sel.addRange(range); return true;
  }
  ...
  function restoreReplyEditorAfterEmojiInsert() {
    ...
    window.requestAnimationFrame(() => { isInsertingReplyEmoji = true; replyEditor.focus(); isInsertingReplyEmoji = false; restoreReplySelection(); saveReplySelection(); syncReplyFormatButtons(); });
  }
  function ensureReplyEmojiLibraryPicker() {
    ...
    replyEmojiLibraryPicker.on('emoji', (selection) => { const emoji = typeof selection === 'string' ? selection : selection?.emoji; if (!emoji) return; shouldRestoreReplyEditorAfterEmojiInsert = true; insertReplyEmoji(emoji); closeEmojiPicker(); restoreReplyEditorAfterEmojiInsert(); });
    replyEmojiLibraryPicker.on('hidden', () => { replyEmojiButton?.setAttribute('aria-expanded','false'); if (shouldRestoreReplyEditorAfterEmojiInsert) { restoreReplyEditorAfterEmojiInsert(); return; } saveReplySelection(); });
    return replyEmojiLibraryPicker;
  }
  function applyReplyFormat(format) {
    if (!replyEditor) return;
    const sel = window.getSelection();
    const inEditor = sel && sel.rangeCount > 0 && replyEditor.contains(sel.getRangeAt(0).commonAncestorContainer);
    if (!inEditor) {
      replyEditor.focus();
      if (!restoreReplySelection()) {
        const s = window.getSelection(), r = document.createRange();
        r.selectNodeContents(replyEditor); r.collapse(false);
  ...
  function syncReplyFormatButtons() {
    ...
    replyFormatButtons.forEach(btn => {
      ...
      const active = document.queryCommandState(fmt);
  ...
  function closeEmojiPicker() { if (!replyEmojiLibraryPicker) return; if (replyEmojiLibraryPicker.pickerVisible) replyEmojiLibraryPicker.hidePicker(); replyEmojiButton?.setAttribute('aria-expanded','false'); restoreReplyEditorAfterEmojiInsert(); }
  function toggleEmojiPicker() {
    const p = ensureReplyEmojiLibraryPicker(); if (!p || !replyEmojiButton) return; saveReplySelection();
    if (p.pickerVisible) { p.hidePicker(); replyEmojiButton.setAttribute('aria-expanded','false'); } else { replyEmojiButton.setAttribute('aria-expanded','true'); p.showPicker(replyEmojiButton); }
  }
  ...
  function populateReplyModal(button) {
    const card = button.closest('.communityPostCard'); if (!card) return;
    const handle = card.querySelector('.postHandle')?.textContent?.trim() ?? '';
    if (replyContextButton) replyContextButton.textContent = handle ? `${handle} 님에게 보내는 답글` : '답글을 보낼 게시물을 찾지 못했습니다.';
    if (replySourceAvatar) replySourceAvatar.src = card.querySelector('.postAvatar img')?.getAttribute('src') ?? '';
  ...
  function openReplyModal(button) {
    ...
    closeEmojiPicker();
    replyEditor.textContent = ''; savedReplySelection = null; savedReplySelectionOffsets = null;
    selectedLocation = null; pendingLocation = null; selectedTaggedUsers = []; pendingTaggedUsers = []; selectedProduct = null;
    if (replyProductButton) replyProductButton.disabled = false;
    replyModalOverlay?.querySelector('[data-selected-product]')?.remove();
  ...
  function closeReplyModal(options = {}) {
    ...
    shouldRestoreReplyEditorAfterEmojiInsert = false;
    replyModalOverlay.hidden = true; replyModalOverlay.classList.remove('tweet-modal-overlay--compose');
    document.body.classList.remove('modal-open');
  ...
  function updateReplyCount(button) {
    const cnt = button.querySelector('.tweet-action-count'); if (!cnt) return;
  ...
  function buildDraftCheckbox(sel) {
    const cb = document.createElement('span'); cb.className = `draft-panel__checkbox${sel ? ' draft-panel__checkbox--checked' : ''}`; cb.setAttribute('aria-hidden','true');
    cb.innerHTML = '<svg viewBox="0 0 24 24"><g><path d="M9.86 18a1 1 0 01-.73-.31l-3.9-4.11 1.45-1.38 3.2 3.38 7.46-8.1 1.47 1.36-8.19 8.9A1 1 0 019.86 18z"></path></g></svg>'; return cb;
  }
  ...
  function renderDraftPanel() {
    if (!draftView) return; const hasItems = getDraftItems().length > 0;
  ...
  function loadDraftIntoComposer(item) { if (!item || !replyEditor) return; replyEditor.textContent = getTextContent(item.querySelector('.draft-panel__text')); closeDraftPanel({ restoreFocus:false }); syncReplySubmitState(); saveReplySelection(); window.requestAnimationFrame(() => replyEditor.focus()); }
  ...
  function renderSelectedProduct() {
    ...
    const card = document.createElement('div'); card.setAttribute('data-selected-product',''); card.className = 'tweet-modal__selected-product';
  ...
  // ===== Init =====
  renderLocationList(); syncLocationUI(); syncUserTagTrigger(); ensureReplyEmojiLibraryPicker();
  ...
  const syncEditorState = () => { saveReplySelection(); syncReplyFormatButtons(); };
  replyEditor?.addEventListener('keyup', syncEditorState);
  replyEditor?.addEventListener('mouseup', syncEditorState);
  replyEditor?.addEventListener('focus', syncEditorState);
  ...
  replyAttachmentMedia?.addEventListener('click', (e) => {
    const rm = e.target.closest('[data-attachment-remove-index]'); if (rm) { const ri = Number.parseInt(rm.getAttribute('data-attachment-remove-index') ?? '-1', 10); if (ri >= 0) removeReplyAttachment(ri); syncReplySubmitState(); return; }
    const eb = e.target.closest('[data-attachment-edit-index]'); if (eb) { pendingAttachmentEditIndex = Number.parseInt(eb.getAttribute('data-attachment-edit-index') ?? '-1', 10); if (replyFileInput) replyFileInput.value = ''; replyFileInput?.click(); }
  });
  ...
  replyLocationList?.addEventListener('click', (e) => { const lb = e.target.closest('.tweet-modal__location-item'); if (!lb) return; const loc = getTextContent(lb.querySelector('.tweet-modal__location-item-label')); if (loc) { applyLocation(loc); closeLocationPanel(); } });
  replyTagChipList?.addEventListener('click', (e) => { const cb = e.target.closest('[data-tag-remove-id]'); if (!cb) return; const uid = cb.getAttribute('data-tag-remove-id'); pendingTaggedUsers = pendingTaggedUsers.filter(u => u.id !== uid); renderTagChipList(); runTagSearch(); replyTagSearchInput?.focus(); });
  replyTagResults?.addEventListener('click', (e) => { const ub = e.target.closest('[data-tag-user-id]'); if (!ub || ub.hasAttribute('disabled')) return; const uid = ub.getAttribute('data-tag-user-id'), user = currentTagResults.find(u => u.id === uid); if (!user || pendingTaggedUsers.some(u => u.id === user.id)) return; pendingTaggedUsers = [...pendingTaggedUsers, { ...user }]; renderTagChipList(); if (replyTagSearchInput) replyTagSearchInput.value = ''; renderTagResults([]); replyTagSearchInput?.focus(); });
  ...
  draftList?.addEventListener('click', (e) => { const item = e.target.closest('.draft-panel__item'); if (!item) return; if (draftPanelState.isEditMode) { toggleDraftSelection(item); renderDraftPanel(); return; } loadDraftIntoComposer(item); });
  ...
  productSelectList?.addEventListener('click', (e) => {
    ...
    productSelectList.querySelectorAll('.draft-panel__item--selected').forEach(el => { el.classList.remove('draft-panel__item--selected'); el.setAttribute('aria-pressed','false'); el.querySelector('.draft-panel__checkbox')?.classList.remove('draft-panel__checkbox--checked'); });
    if (!wasSelected) { item.classList.add('draft-panel__item--selected'); item.setAttribute('aria-pressed','true'); item.querySelector('.draft-panel__checkbox')?.classList.add('draft-panel__checkbox--checked'); }
